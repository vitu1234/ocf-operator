...
func (r *AutomationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	case "Numeric":
		...
		if raw_device_resource_property.Measurement != nil {
			var time_period int64 = 0
			if trigger.Period.Hours != nil {
				hour := *trigger.Period.Hours
				seconds := hour * 3600
				time_period += seconds
			}

			if trigger.Period.Minutes != nil {
				minute := *trigger.Period.Minutes
				seconds := minute * 60
				time_period += seconds
			}

			if trigger.Period.Seconds != nil {
				time_period += *trigger.Period.Seconds
			}

			//Test trigger conditions conditions
			if trigger.Max != nil && trigger.Min != nil {
				if *raw_device_resource_property.Measurement <= *trigger.Min || *raw_device_resource_property.Measurement >= *trigger.Max {

				}

			} else if trigger.Max == nil && trigger.Min != nil {
				if *raw_device_resource_property.Measurement <= *trigger.Min {

				}
			} else if trigger.Max != nil && trigger.Min == nil {
				if *raw_device_resource_property.Measurement >= *trigger.Max {

				}
			}
		} else if raw_device_resource_property.Value != nil {
			fmt.Println("Trigger is a boolean")
		}
